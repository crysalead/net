...
class Cookie
{
    ...
    public function match($url)
    {
        ...
        if (!$this->matchScheme($infos['scheme'])) {
            return false;
        }

        if (!$this->matchDomain($infos['host'])) {
            return false;
        }

        return $this->matchPath($infos['path']);
    }

    /**
     * Checks if a path match the SetCookie path.
     *
    ...
    /**
     * Checks if a domain match the SetCookie scheme.
     *
     * @param  string  $scheme The scheme name.
     * @return boolean         Returns `true` if schemes match, `false otherwise`.
     */
